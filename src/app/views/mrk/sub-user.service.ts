...
export class SubUserService extends BaseService<UserDataModel, number>  {
  ...
  constructor(public _http: HttpClient, private _auth: AuthenticateService) {     super(_http, environment.api.baseUrl);

}
GetChildsLevelOne(
  userID: number,
): Observable<Result<UserDataModel[]>> {
  ...
  return this._http.get<Result<UserDataModel[]>>(
    this._base +
      '/Relation/GetChildsLevelOne/'+'?userId=' +
      userID,
    _options
  );
}
