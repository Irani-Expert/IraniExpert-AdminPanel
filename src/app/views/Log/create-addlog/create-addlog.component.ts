...
export class CreateAddlogComponent implements OnInit {
  ...
  firstSetup() {
    this._logServices
      .getAllTableType()
      .subscribe((res: Result<TableType[]>) => {
        if (res.success) {
  ...
  getAllLogs(pageIndex: number, pageSize: number) {
    this._logServices
      .getAllLog(pageIndex, pageSize)
      .subscribe((res: Result<AllCheckingLog[]>) => {
        if (res.success) {
  ...
  addNodesTochild() {
    this.nodes.forEach((node) => {
      ...
      this.data.forEach((nodeChilderen) => {
        if (
          nodeChilderen.tableType == Number(node.key) ||
          node.requestType == nodeChilderen.requestType
        ) {
          saveChilderen.push({
            key: '0-' + counter,
            label: nodeChilderen.title,
            parentkey: Number(node.key),
            nodeCount: nodeChilderen.loggingCount,
            data: 'https://angular.io',
            type: 'url',
            id: nodeChilderen.id,
            checked: nodeChilderen.isActive,
          });
          counter++;
        }
      });
  ...
  OpenModal(content: any) {
    this.modalService
      ...
      .result.then(
        (result: boolean) => {
          this.addNewLogModel.requestType = Number(
            this.addNewLogModel.requestType
          );
          this.addNewLogModel.tableType = Number(this.addNewLogModel.tableType);

          this._logServices
            .create(this.addNewLogModel, 'MainLogging')
            .subscribe((data) => {
              if (data.success) {
    ...
    this.modalService
      ...
      .result.then(
        (result: boolean) => {
          ...
          this.addNewLogModel.tableType = Number(this.addNewLogModel.tableType);
          this._logServices
            .updateList(this.addNewLogModel)
            .subscribe((res: Result<AllCheckingLog>) => {
              if (res.success) {
                let finder = this.data.findIndex((finder) => finder.id == id);
                if (finder != -1) {
  ...
  nodeSelect(event) {
    let findIndex = this.opendNodeList.findIndex(
      (finder) => finder.key == event.node.key
    );
    if (findIndex != -1) {
